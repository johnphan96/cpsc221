...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++)
  {
    for (unsigned y = 0; y < image.height(); y++)
    {
      ...
      int distanceX = (int) x - centerX;
      int distanceY = (int) y - centerY;

      double euclidDistance = sqrt((distanceX*distanceX) + (distanceY*distanceY));
      if (euclidDistance > 200) 
      {
        pixel->l = 0;
      } else
      {
        double luminanceScaleFactor = 1 - euclidDistance * 0.005;
        pixel->l = pixel->l * luminanceScaleFactor;
      }
    }
  }
  return image;
...
PNG ubcify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++)
  {
    ...
    {
      HSLAPixel *pixel = image.getPixel(x, y);
      double ubcYellow = 40.0;
      double ubcBlue = 210.0;
      double diffYellow = sqrt((pixel->h - ubcYellow) * (pixel->h - ubcYellow));
      double diffBlue = sqrt((pixel->h - ubcBlue) * (pixel->h - ubcBlue));
      if (diffYellow < diffBlue)
      {
        pixel->h = ubcYellow;
      } else
      {
