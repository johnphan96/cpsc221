...
Node *copy(Node *original)
{
  ...
  copy->next = NULL;
  Node *curr_original = original->next;
  Node *curr_copy = copy;

  while (curr_original != NULL)
  {
    insert_after(copy, curr_copy->key, curr_original->key);
...
Node *interleave(Node *list1, Node *list2)
{
  ...
  {
    Node *tempList = copy(list2);
    list1->next = interleave(tempList, list1->next);
  }
